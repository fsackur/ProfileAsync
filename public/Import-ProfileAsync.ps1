    ...
    $null = Register-ObjectEvent @HandlerParams -Action {
        ...
        if ($Powershell.InvocationStateInfo.State -ge 2)
        {
            ...
            if ($VerbosePresent)
            {
                ...
                "VERBOSE: Asynchronous execution $($State.ToLower())" | Write-Host -ForegroundColor Yellow
            }
        }
    }
}
